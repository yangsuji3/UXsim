...
if __name__ == "__main__":
    ...
    W = World(
        name="test",    # Scenario name. Can be blank. Used as the folder name for saving results.
    ...
    for t in list(range(0,W.TMAX,int(W.TMAX/6))):
        W.analyzer.network(t, detailed=0, network_font_size=1, figsize=(4,4))
    for t in list(range(0,W.TMAX,int(W.TMAX/6))):
        W.analyzer.network(t, detailed=1, network_font_size=1)
    ...
    W.analyzer.network_fancy(animation_speed_inverse=15, sample_ratio=0.3, interval=5, trace_length=5, network_font_size=1)
